...
public enum BACK {
    UNKNOW("未知原因出错", 0),
    FUNCTIONADDSUCCESS("添加菜单成功", 1),
    BEANADDSUCCESS("添加因子成功", 20),
    FUNCTIONUPDATESUCCESS("修改菜单成功", 2),
    BEANUPDATESUCCESS("修改因子成功", 21),
    FUNCTIONISEXIST("该标识的菜单已存在", 11),
    BEANISEXIST("该标识的因子已存在", 22),
    FUNCTIONNOTHASROLE("菜单至少要有一个角色", 15),
    BEANNOTHASROLE("因子至少要有一个角色", 23),
    ...
    FUNCTIONNOTHASSYSTEN("菜单所属系统为空", -1),
    BEANNOTHASSYSTEN("因子所属系统为空", -21),
    ROLENOTHASSYSTEN("角色所属系统为空", -2),
    USERNOTHASSYSTEN("用户所属系统为空", -3),
    RESOURCENOTHASSYSTEN("资源所属系统为空", -4),
    FUNCTIONCYCLE("你不可以选择你正要修改菜单的下级菜单作为其上一级菜单", -7);
