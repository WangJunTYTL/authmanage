...
当前sdk包只有java版本，除了依赖Ehcache包为了实现客户端的缓存外，不依赖任何第三方包，所以只要
你当前的开发语言可以和java语言相互调用就可适用于用你的系统，比如当前我们把sdk包用与spring编写
...
内容是客户端可以通过springmvc HandlerInterceptorAdapter 拦截所有请求，然后通过检查controller上的注解
